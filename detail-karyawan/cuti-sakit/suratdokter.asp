...
' of a directory with write permissions, for example "C:\Inetpub\wwwroot"
 ...
 dim area
	area = request("area")
	if area = "" then
		response.AddHeader "REFRESH","0:URL=suratdokter.asp?nip=000000000"
	end if

  uploadsDirVar = "D:\newsite\hrd\suratdokter\"
...
<HEAD>
<head>
<TITLE>UPLOAD SURAT DOKTER</TITLE>
...
<div class="container">
    <div class='row'>
        <div class='col text-center'>
            <h3>UPLOAD SURAT DOKTER</h3>
        </div>
    </div>
    <div class="upload">
        <form name="frmSend" method="POST" enctype="multipart/form-data" action="suratdokter.asp?nip=<%=request.querystring("nip")%>&id=<%=request.querystring("id")%>" onSubmit="return onSubmitForm(this);">   	<p style="margin-top: 0; margin-bottom: 0">&nbsp;</p>
        ...
        if Request.ServerVariables("REQUEST_METHOD") <> "POST" then
			...
			response.redirect "updateFieldSuratDokterDatabase.asp?nip=" & request.querystring("nip") & "&id=" & request.querystring("id")
        end if

        %>
        <u><b>Ketentuan :</b></u><ul>
        <li>Pastikan nama file Photo sudah sesuai dengan ID Cuti yang bersangkutan.</li>
        <li>CONTOH : 0010821003.jpg</li>
        <li>Kami hanya menerima foto dalam bentuk format file *.jpg</li>

        <button type="button" onclick="window.location.href='../cutiSakitIzin.asp?nip=<%= nip %>'" class="btn btn-danger mt-4">Kembali</button>

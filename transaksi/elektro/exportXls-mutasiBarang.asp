...
<% 
    ...
    Response.AddHeader "content-disposition", "filename=LAPORANPINJAMANBARANGELEKTRONIK "& Request.QueryString("tahun") &".xls"
    ...
    if bulan = "" then
        ...
        if radio = "detail" then
            if nama = "" then
                ...
                if bulan = "1" then
                    mutasi_cmd.commandText = "SELECT SAPK_NIP, SAPK_Awal, (SAPK_Awal + SAPK_Pinjam01) - (SAPK_Bayar01) AS tsaldoakhir, HRD_M_Karyawan.Kry_nama FROM HRD_T_SA_PK_Elektronik LEFT OUTER JOIN HRD_M_Karyawan ON HRD_T_SA_PK_Elektronik.SAPK_Nip = HRD_M_Karyawan.Kry_Nip WHERE(SAPK_Tahun = '"& tahun &"') and isnull(sapk_nip,'') <> '' AND (sapk_awal + SAPK_Pinjam01) <> 0 AND HRD_M_Karyawan.Kry_nama LIKE '%"& nama &"%' GROUP BY SAPK_NIP, SAPK_Awal, (SAPK_Awal + (SAPK_Pinjam01)) - (SAPK_Bayar01), HRD_M_Karyawan.Kry_nama ORDER BY HRD_M_Karyawan.Kry_Nama ASC"
...
<head>
    ...
    <% if radio = "detail" then  %>
        <tr>
            <td colspan="6" style="text-align:center;"><b><u>PINJAMAN DAN PEMBAYARAN CICILAN BARANG DETAIL</u></b></td>
        </tr>
        <%if bulan <> "" then%>
            <tr>
        ...
        if nama = "" then
            ...
            saldoakhir = 0
            do until mutasi.eof 
                mutasi_cmd.commandText = "SELECT TPK_Tanggal, TPK_ID_Elektronik, TPK_Ket, TPK_PP FROM HRD_T_PK_Elektronik WHERE "& filterBln &" Year(TPK_Tanggal) = '"& tahun &"' AND TPK_AktifYN = 'Y' AND TPK_Nip = '"& mutasi("SAPK_Nip") &"'"
                            ' Response.Write mutasi_cmd.commandText & "<br>"
                set pk = mutasi_cmd.execute

                mutasi_cmd.commandText = "SELECT TPK_Tanggal, TPK_ID_Elektronik, TPK_Ket, TPK_PP FROM HRD_T_BK_Elektronik WHERE "& filterBln &" Year(TPK_Tanggal) = '"& tahun &"' AND TPK_AktifYN = 'Y' AND TPK_Nip = '"& mutasi("SAPK_Nip") &"'"
        ...
        %>
            ...
            %>
                <tr>
                        ...
                        <td style="mso-number-format:\@;"><%= pk("TPK_ID_Elektronik") %></td>
                        <td><%= pk("TPK_Ket") %></td>   
            ...
            <%
                ...
                <tr>
                    ...
                    <td style="mso-number-format:\@;"><%= bk("TPK_ID_Elektronik") %></td>
                    <td><%= bk("TPK_Ket") %></td>   
                    <td></td>
    ...
    <% else %>
        <tr>
            <td colspan="6" style="text-align:center;"><b><u>PINJAMAN DAN PEMBAYARAN CICILAN BARANG REKAPITULASI</u></b></td></br>
        </tr>
        <%if bulan <> "" then%>
            <tr>
                <td colspan="6" style="text-align:center;">Priode Bulan <%= MonthName(bulan) &" Tahun "& tahun%></td>

...
<head>
    ...
    <script>
      ...
      function updatePembayaran(id,tgl,nip,nama,ket,pp){
        // label untuk lama dan cicilan ke
        let laschar = ket.substring(ket.length - 5);
        let arraychar = laschar.replace(/^\s+|\s+$/gm,'').split("/");

        $('#tgl').attr("type","text");
        $('#tgl').attr("readonly",true);
        $(".cariKeterangan").html(`<textarea class="form-control areaKeterangan" id="keterangan" name='keterangan' readonly>${ket}</textarea>`);
        $('#nomor').val(id);
        $('#tgl').val(tgl);
        $('#nip').val(nip);
...
<div class='container'>
    <div class='row'>
        <div class='col text-center mt-3'>
            <h3>PEMBAYARAN CICILAN ELEKTRONIK</h3>
        </div>
    </div>
    <div class='row'>
      <div class='col'>
        ...
        <div class='col'>
            ...
            %>
						<% if requestrecords <> 0 then %>
							<a class="page-link" href="index.asp?offset=<%= requestrecords - recordsonpage%>&Page=<%=npage%>&tgla=<%= tgla %>&tgle=<%= tgle %>&nama=<%= nama %>&nip=<%= nip %>&area=<%= area %>&ckTgl=<%= ckTgl %>&ckNama=<%= ckNama %>&ckArea=<%= ckArea %>">&#x25C4; Previous </a>
						<% else %>
...
<% end if %>
<!-- Modal -->
<div class="modal fade modalPinjaman" id="modalPembayaran" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <form action='pembayaran_add.asp' method='post'>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-3">
            <input type="number" class="form-control form-control-sm  m-0" id="nip" name='nip' placeholder="Nip" readonly required>
          </div>
          <div class="col-sm-5">
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-8 cariKeterangan"></div>
        </div>
        <!--
        <div class='row justify-content-end'>
         ...
         -->
        <div class="row mb-3">
          <label class="col-sm-4 col-form-label col-form-label-sm">Pembayaran Ke</label>
          <div class="col-sm-3">
            <input type="text" class="form-control form-control-sm" name="pembayaranke" id="pembayaranke" readonly required>
          </div>
          <label class="col-sm-1 col-form-label col-form-label-sm">Lama</label>
          <div class="col-sm-2">
            <input type="text" class="form-control form-control-sm" name="inplama" id="inplama" readonly required>
          </div>
        </div>
        <div class="row mb-3">
          <label for="cicilan" class="col-sm-4 col-form-label col-form-label-sm">Cicilan</label>
          <div class="col-sm-4">
            <input type="text" class="form-control form-control-sm" id="cicilan" name='cicilan' value="0" autocomplete="off" onchange="rupiah($(this).val())" required>
            <input type="hidden" class="form-control form-control-sm" id="id" name='id' value ="<%= session("username") %>" >
          </div>
          <div class="col-sm-2">
            <button class="btn btn-primary btn-sm" type="button" onclick="return lunas()">Lunas</button>
          </div>
...
<script>
    ...
    function lunas(){
      ...
      let keterangan = $("#keterangan").val();
        ...
        if (nip == "" || keterangan == "" ){
          alert("Mohon Pilih Nama Karyawan dan Hutang Yang ingin di lunasi!!");
        }else{
          str = keterangan.substring(0,19);
          $.get(`getlunas.asp?nip=${nip}&id=${str}`, function(data){
            $("#cicilan").val(data);
          })
        }
    }

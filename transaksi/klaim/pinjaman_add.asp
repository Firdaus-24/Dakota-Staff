...
pinjaman_cmd.commandText = "SELECT * FROM HRD_T_PK WHERE TPK_Nip = '"& nip &"' and TPK_Tanggal = '"& tgl &"' AND TPK_Ket = '"& keterangan &"' AND TPK_PP = '"& hutang &"' and TPK_AktifYN = 'Y'"
...
if pinjaman.eof then
    ...
    update_cmd.commandText = "SELECT TPK_ID FROM HRD_T_PK WHERE TPK_Tanggal = '"& tgl &"' AND TPK_Nip = '"& nip &"' AND TPK_Ket = '"& keterangan &"' AND TPK_PP = '"&  hutang &"' AND TPK_Lama = '"& lama &"' AND TPK_Bunga = '"& bunga &"'"
    set data = update_cmd.execute
    
    if not data.eof then
        update_cmd.commandText = "UPDATE HRD_T_PK SET TPK_PotongGajiYN = '"& potgaji &"' WHERE TPK_Nip = '"& nip &"' AND TPK_Tanggal = '"& tgl &"' AND TPK_Ket = '"& keterangan &"' AND TPK_PP = '"& hutang &"' AND TPK_Lama = '"& lama &"'"
        update_cmd.execute
    end if
    ...
    keterangan = "TAMBAH KLAIM PINJAMAN KARYAWAN UNTUK NIP ("& nip &") DI PROSES "& Cdate(tgl) 
    ...
    Response.Write "<div class='notiv-gagal' data-aos='fade-up'><span>Data Sudah Terdaftar</span><img src='../../logo/gagal_dakota.PNG'><a href='pinjamanKaryawan.asp' class='btn btn-primary'>kembali</a></div>"
end if

...
<% 
  ...
  ' select area aktif
  pinjaman_cmd.commandText = "SELECT Agen_id, Agen_Nama FROM HRD_M_Karyawan INNER JOIN HRD_T_PK ON HRD_M_Karyawan.Kry_Nip = HRD_T_PK.TPK_Nip LEFT OUTER JOIN GLB_M_Agen ON GLB_M_Agen.Agen_ID = HRD_M_Karyawan.Kry_AgenID WHERE GLB_M_Agen.Agen_AktifYN = 'Y' AND HRD_M_Karyawan.Kry_AktifYN = 'Y' GROUP BY Agen_id, Agen_Nama ORDER BY Agen_Nama ASC"
...
<head>
    ...
    <script>
      // function rubah format currency
      ...
      function aktifPinjaman(id,p,nip) {
        if (confirm("YAKIN UNTUK DI RUBAH??") === true ){
          return window.location.href = "aktif.asp?id="+id+"&p="+p+"&nip="+nip
        }else{
          return false;
        }
      }
        ...
        <div class='col'>
            ...
            <thead>
                <tr>
                  ...
                  <%if session("HT1AB") = true OR session("HT1AC") = true OR session("HT1AD") = true then%>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                      <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                        <%if session("HT1AB") = true then%>
                          <button type="button" class="btn btn-primary btn-sm py-0 px-2" onclick="return updatePinjaman('<%=rs("TPK_ID")%>','<%= rs("TPK_Nip") %>','<%= rs("TPK_Tanggal") %>','<%= rs("Kry_Nama") %>','<%= rs("TPK_ket") %>','<%= rs("TPK_pp") %>','<%= rs("TPK_Bunga") %>','<%= rs("TPK_lama") %>','<%= rs("TPK_PotongGajiYN") %>')" data-bs-toggle="modal" data-bs-target="#modalPimjaman">Edit</button>
                        <%end if%>
                        <% if session("HT1AC") = true then%>
                          <% if rs("TPK_AktifYN") = "Y" then %>
                            <button type="button" class="btn btn-danger btn-sm py-0 px-2" onclick="return aktifPinjaman('<%=rs("TPK_ID")%>','<%= rs("TPK_AktifYN") %>','<%= rs("TPK_Nip") %>')">NonAktif</button>
                          <% else %>
                            <button type="button" class="btn btn-warning btn-sm py-0 px-2" onclick="return aktifPinjaman('<%=rs("TPK_ID")%>','<%= rs("TPK_AktifYN") %>','<%= rs("TPK_Nip") %>')">Aktif</button>
                          <% end if %>
                        <%end if%>
                        <%if session("HT1AD") = true then%>
...
<script>
  $('#nama').on('keyup', function (result) {
    if($("#nama").val() == ""){
      ...
      $.get(`cariKaryawanPinjam.asp?key=${$('#nama').val().toUpperCase().replace(' ', '%20')}`, function (data) {

...
<% 
    ...
    update_cmd.commandText = "SELECT * FROM HRD_T_BK_Pribadi WHERE TPK_Nip = '"& nip &"' AND TPK_ID_Pribadi = '"& nomor &"' AND TPK_AktifYN = 'Y'"
    set data = update_cmd.execute

    if not data.eof then
        update_cmd.commandText = "UPDATE HRD_T_BK_Pribadi SET TPK_Tanggal = '"& tgl &"', TPK_Ket = '"& keterangan &"', TPK_PP = "& pp &", TPK_UpdateID = '"& id &"', TPK_UpdateTime = GETDATE() WHERE TPK_ID_Pribadi = '"& nomor &"' and TPK_Nip = '"& nip &"'"
        ...
        keterangan = "UPDATE PEMBAYARAN PRIBADI UNTUK NIP ("& nip &") DENGAN NOMOR ("& nomor &") "

...
pinjaman_cmd.commandText = "SELECT * FROM HRD_T_PK_PRIBADI WHERE TPK_Nip = '"& nip &"' and TPK_Tanggal = '"& tgl &"' AND TPK_Ket = '"& keterangan &"' AND TPK_PP = '"& hutang &"' and TPK_AktifYN = 'Y'"

set pinjaman = pinjaman_cmd.execute

if pinjaman.eof then
    pinjaman_cmd.commandText = "exec sp_AddHRD_T_PK_PRIBADI '"& kode &"','"& tgl &"','"& nip &"','"& keterangan &"',"& hutang &","& bunga &","& lama &",'"& session("username") &"'"
    ...
    update_cmd.commandText = "SELECT TPK_ID_PRIBADI FROM HRD_T_PK_PRIBADI WHERE TPK_Tanggal = '"& tgl &"' AND TPK_Nip = '"& nip &"' AND TPK_Ket = '"& keterangan &"' AND TPK_PP = '"&  hutang &"' AND TPK_Lama = '"& lama &"' AND TPK_Bunga = '"& bunga &"'"
    set data = update_cmd.execute
    
    if not data.eof then
        update_cmd.commandText = "UPDATE HRD_T_PK_PRIBADI SET TPK_PotongGajiYN = '"& potgaji &"' WHERE TPK_Nip = '"& nip &"' AND TPK_Tanggal = '"& tgl &"' AND TPK_Ket = '"& keterangan &"' AND TPK_PP = '"& hutang &"' AND TPK_Lama = '"& lama &"'"
        update_cmd.execute
    end if
    ...
    keterangan = "TAMBAH PINJAMAN PRIBADI UNTUK NIP ("& nip &") DI PROSES "& Cdate(tgl) 

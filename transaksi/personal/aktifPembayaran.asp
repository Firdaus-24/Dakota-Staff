...
if i = "Y" then
    aktif.commandText = "UPDATE HRD_T_BK_Pribadi SET TPK_AktifYN = 'N' WHERE TPK_ID_Pribadi = '"& p &"' AND TPK_Nip = '"& nip &"'"
    ...
    keterangan = "NONAKTIF PEMBAYARAN PRIBADI KARYAWAN ("& nip &") DENGAN NOMOR (" & p &")"
    call updateLog(eventt,url,key,session("username"),session("server-id"),dateTime,ip,browser,keterangan)
else
    aktif.commandText = "UPDATE HRD_T_BK_Pribadi SET TPK_AktifYN = 'Y' WHERE TPK_ID_Pribadi = '"& p &"' AND TPK_Nip = '"& nip &"'"
    ' Response.Write aktif.commandText
    aktif.execute
    ...
    keterangan = "AKTIF PEMBAYARAN PRIBADI KARYAWAN ("& nip &") DENGAN NOMOR (" & p &")"
    call updateLog(eventt,url,key,session("username"),session("server-id"),dateTime,ip,browser,keterangan)
end if

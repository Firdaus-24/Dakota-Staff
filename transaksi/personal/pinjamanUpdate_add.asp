...
update_cmd.commandText = "SELECT * FROM HRD_T_PK_PRIBADI WHERE TPK_ID_PRIBADI = '"& nomor &"' AND TPK_Nip = '"& nip &"' AND TPK_AktifYN = 'Y'"
set data = update_cmd.execute

if not data.eof then
    update_cmd.commandText = "UPDATE HRD_T_PK_PRIBADI SET TPK_Tanggal = '"& tgl &"', TPK_PP = '"& hutang &"', TPK_Bunga = '"& bunga &"', TPK_Lama = '"& lama &"', TPK_updateID = '"& id &"', TPK_UpdateTIme = '"& date() &"', TPK_PotongGajiYN = '"& potgaji &"' WHERE TPK_ID_PRIBADI = '"& nomor &"'"
    ...
    keterangan = "UPDATE PINJAMAN PRIBADI UNTUK NIP ("& nip &") DENGAN NOMOR "& nomor

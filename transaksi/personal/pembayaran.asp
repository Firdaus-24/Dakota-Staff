...
<% 
    ...
    query = "SELECT HRD_M_Karyawan.Kry_Nama, HRD_T_BK_Pribadi.TPK_ID_Pribadi, HRD_T_BK_Pribadi.TPK_Tanggal, HRD_T_BK_Pribadi.TPK_Nip, HRD_T_BK_Pribadi.TPK_Ket, HRD_T_BK_Pribadi.TPK_PP, HRD_T_BK_Pribadi.TPK_AktifYN, HRD_T_BK_Pribadi.TPK_UpdateID, HRD_T_BK_Pribadi.TPK_UpdateTime FROM HRD_M_Karyawan RIGHT OUTER JOIN HRD_T_BK_Pribadi ON HRD_M_karyawan.Kry_Nip = HRD_T_BK_Pribadi.TPK_Nip LEFT OUTER JOIN GLB_M_Agen ON HRD_M_Karyawan.Kry_AgenID = GLB_M_Agen.Agen_ID WHERE HRD_M_Karyawan.kry_AktifYN = 'Y'"

    if tgla <> "" And tgle <> "" then
        filterTgl =  " AND HRD_T_BK_Pribadi.TPK_tanggal BETWEEN '"& Cdate(tgla) &"' AND '"& Cdate(tgle) &"'"
    else 
        filterTgl = ""
    end if
    ...
    orderBy = " ORDER BY HRD_T_BK_Pribadi.TPK_Tanggal DESC" 
    ...
    pembayaran_cmd.commandText = "SELECT Agen_id, Agen_Nama FROM HRD_M_Karyawan INNER JOIN HRD_T_BK_Pribadi ON HRD_M_Karyawan.Kry_Nip = HRD_T_BK_Pribadi.TPK_Nip LEFT OUTER JOIN GLB_M_Agen ON GLB_M_Agen.Agen_ID = HRD_M_Karyawan.Kry_AgenID WHERE GLB_M_Agen.Agen_AktifYN = 'Y' GROUP BY Agen_id, Agen_Nama ORDER BY Agen_Nama ASC"
...
<head>
    ...
    <title>PEMBAYARAN PRIBADI</title>
...
<div class='container'>
    <div class='row'>
        <div class='col text-center mt-3'>
            <h3>PEMBAYARAN PINJAMAN PRIBADI</h3>
        </div>
    </div>
    <div class='row'>
        <div class='col'>
        ...
        <div class='col'>
            <table class="table tablePembayaran">
                <thead>
                    ...
                    <%if session("HT4BB") = true OR session("HT4BC") = true OR session("HT4BD") = true then %>
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                            <%if session("HT4BB") = true then%>
                                <button type="button" class="btn btn-primary btn-sm py-0 px-2" onclick="return updatePembayaran('<%=rs("TPK_ID_Pribadi")%>','<%= rs("TPK_tanggal") %>','<%= rs("TPK_Nip") %>','<%= rs("Kry_Nama") %>','<%= rs("TPK_Ket") %>','<%= rs("TPK_PP") %>')" data-bs-toggle="modal" data-bs-target="#modalPembayaran">Edit</button>
                            <%end if%>
                            <%if session("HT4BC") = true then%>
                                <% if rs("TPK_AktifYN") = "Y" then %>
                                    <button type="button" class="btn btn-danger btn-sm py-0 px-2" onclick="return aktifPembayaran('<%=rs("TPK_ID_Pribadi")%>','<%= rs("TPK_AktifYN") %>','<%= rs("TPK_Nip") %>')">NonAktif</button>
                                <% else %>
                                    <button type="button" class="btn btn-warning btn-sm py-0 px-2" onclick="return aktifPembayaran('<%=rs("TPK_ID_Pribadi")%>','<%= rs("TPK_AktifYN") %>','<%= rs("TPK_Nip") %>')">Aktif</button>
                                <% end if %>
                            <% end if %>
                            <%if session("HT4BD") = true then%>
                                <button type="button" class="btn btn-secondary btn-sm py-0 px-2" onclick="window.open('EXPORT-Pembayaran.asp?p=<%= rs("TPK_ID_Pribadi") %>')">Cetak</button>
                            <%end if%>
                        </div>
                    </td>
                </tr>

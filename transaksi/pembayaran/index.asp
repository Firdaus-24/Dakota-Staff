    ...
    <script>
      ...
      function aktifPembayaran(e,i){
        if (confirm("ANDA YAKIN UNTUK MERUBAH???") == true){
          window.location.href="aktif.asp?p="+e+"&i="+i
        }
      }
      function getTgl(){
...
<div class='container'>
    <div class='row'>
        <div class='col text-center mt-3'>
            <h3>PEMBAYARAN PINJAMAN KARYAWAN</h3>
        </div>
    </div>
    <div class='row'>
      <div class='col'>
    ...
    <div class="header p-2 mt-2">
      <div class='row'>
      <form action="index.asp" method="post" id="form-cari">
        <div class='col'>
          <div class="mb-3 row ">
    ...
    <div class='row'>
        <div class='col'>
            ...
            if Cint(page) = pagelistcounter then
					%>	
						<a class="page-link hal d-flex bg-primary text-light" href="index.asp?offset=<%= pagelist %>&Page=<%=pagelistcounter%>&tgla=<%= tgla %>&tgle=<%= tgle %>&nama=<%= nama %>&nip=<%= nip %>&area=<%= area %>&ckTgl=<%= ckTgl %>&ckNama=<%= ckNama %>&ckArea=<%= ckArea %>"><%= pagelistcounter %></a>   
					<% else %>
						<a class="page-link hal d-flex" href="index.asp?offset=<%= pagelist %>&Page=<%=pagelistcounter%>&tgla=<%= tgla %>&tgle=<%= tgle %>&nama=<%= nama %>&nip=<%= nip %>&area=<%= area %>&ckTgl=<%= ckTgl %>&ckNama=<%= ckNama %>&ckArea=<%= ckArea %>"><%= pagelistcounter %></a>   
          <% end if %>
          <%
            ...
            %>
						<% if(recordcounter > 1) and (lastrecord <> 1) then %>
							<a class="page-link next" href="index.asp?offset=<%= requestrecords + recordsonpage %>&Page=<%=page%>&tgla=<%= tgla %>&tgle=<%= tgle %>&nama=<%= nama %>&nip=<%= nip %>&area=<%= area %>&ckTgl=<%= ckTgl %>&ckNama=<%= ckNama %>&ckArea=<%= ckArea %>">Next &#x25BA;</a>
						<% else %>

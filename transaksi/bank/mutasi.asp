    ...
    if bulan = "" then
        ...
        if radio = "detail" then
            if nama = "" then
                ...
                if bulan = "1" then
                    mutasi_cmd.commandText = "SELECT SAPK_NIP, SAPK_Awal, (SAPK_Awal + SAPK_Pinjam01) - (SAPK_Bayar01) AS tsaldoakhir, HRD_M_Karyawan.Kry_nama FROM HRD_T_SA_PK_Bank LEFT OUTER JOIN HRD_M_Karyawan ON HRD_T_SA_PK_Bank.SAPK_Nip = HRD_M_Karyawan.Kry_Nip WHERE(SAPK_Tahun = '"& tahun &"') and isnull(sapk_nip,'') <> '' AND (sapk_awal + SAPK_Pinjam01) <> 0 AND HRD_M_Karyawan.Kry_nama LIKE '%"& nama &"%' GROUP BY SAPK_NIP, SAPK_Awal, (SAPK_Awal + (SAPK_Pinjam01)) - (SAPK_Bayar01), HRD_M_Karyawan.Kry_nama ORDER BY HRD_M_Karyawan.Kry_Nama ASC"
                ...
                <tbody>
                    ...
                    if nama = "" then
                        do while not mutasi.eof 
                            mutasi_cmd.commandText = "SELECT TPK_Tanggal, TPK_ID_Bank, TPK_Ket, TPK_PP FROM HRD_T_PK_bank WHERE "& filterBln &" Year(TPK_Tanggal) = '"& tahun &"' AND TPK_AktifYN = 'Y' AND TPK_Nip = '"& mutasi("SAPK_Nip") &"'"

                            set pk = mutasi_cmd.execute

                            mutasi_cmd.commandText = "SELECT TPK_Tanggal, TPK_ID_Bank, TPK_Ket, TPK_PP FROM HRD_T_BK_Bank WHERE "& filterBln &" Year(TPK_Tanggal) = '"& tahun &"' AND TPK_AktifYN = 'Y' AND TPK_Nip = '"& mutasi("SAPK_Nip") &"'"

...
<% 
    ...
    update_cmd.commandText = "SELECT * FROM HRD_T_BK_Bank WHERE TPK_Nip = '"& nip &"' AND TPK_ID_Bank = '"& nomor &"' AND TPK_AktifYN = 'Y'"
    set data = update_cmd.execute

    if not data.eof then
        update_cmd.commandText = "UPDATE HRD_T_BK_Bank SET TPK_Tanggal = '"& tgl &"', TPK_Ket = '"& keterangan &"', TPK_PP = "& pp &", TPK_UpdateID = '"& id &"', TPK_UpdateTime = GETDATE() WHERE TPK_ID_Bank = '"& nomor &"' and TPK_Nip = '"& nip &"'"
        ...
        keterangan = "UPDATE PEMBAYARAN BANK UNTUK NIP ("& nip &") DENGAN NOMOR ("& nomor &") "

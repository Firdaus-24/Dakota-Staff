...
tambah_cmd.commandText = "SELECT * FROM HRD_T_BK_Bank WHERE TPK_Tanggal = '"& tgl &"' AND TPK_Nip = '"& nip &"' AND TPK_Ket = '"& keterangan &"'"
set tambah = tambah_cmd.execute

kode = mid(nip,1,3)

if tambah.eof then
    tambah_cmd.commandText = "exec sp_AddHRD_T_BK_Bank '"& kode &"','"& tgl &"','"& nip &"','"& keterangan &"','"& cicilan &"','"& id &"'"
    ...
    keterangan = "TAMBAH PEMBAYARAN BANK KARYAWAN UNTUK NIP ("& nip &") DI PROSES ("& tgl &") "

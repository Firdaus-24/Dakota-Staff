...
<% 
  ...
  query = "SELECT HRD_M_Karyawan.Kry_Nama, HRD_T_PK_Bank.TPK_ID_Bank, HRD_T_PK_Bank.TPK_Tanggal, HRD_T_PK_Bank.TPK_Nip, HRD_T_PK_Bank.TPK_Ket, HRD_T_PK_Bank.TPK_PP, HRD_T_PK_Bank.TPK_Bunga, HRD_T_PK_Bank.TPK_Lama, HRD_T_PK_Bank.TPK_AktifYN, HRD_T_PK_Bank.TPK_PotongGajiYN FROM HRD_M_Karyawan LEFT OUTER JOIN HRD_T_PK_Bank ON HRD_M_karyawan.Kry_Nip = HRD_T_PK_Bank.TPK_Nip LEFT OUTER JOIN GLB_M_Agen ON HRD_M_Karyawan.Kry_AgenID = GLB_M_Agen.Agen_ID WHERE HRD_M_Karyawan.Kry_AktifYN = 'Y' AND TPK_ID_Bank IS NOT NULL"

  if tgla <> "" and tgle <> "" then
    filterTgl = " AND HRD_T_PK_bank.TPK_tanggal BETWEEN '"& Cdate(tgla) &"' AND '"& Cdate(tgle) &"'"
  else
    filterTgl = ""
  end if
  ...
  orderBy = " ORDER BY HRD_T_PK_bank.TPK_Tanggal DESC"
    ...
    <div class='row'>
        <div class='col'>
            ...
            <thead class="bg-secondary text-light">
                ...
                <%if session("HT3AB") = true OR session("HT3AC") = true OR session("HT3AD") = true then%>
            ...
            <tbody>
                ...
                %>
                <tr>
                    <th scope="row"><%= rs("TPK_ID_Bank") %></th>
                    ...
                    <td>
                      <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                        <%if session("HT3AB") = true then%>
                          <button type="button" class="btn btn-primary btn-sm py-0 px-2" onclick="return updatePinjaman('<%=rs("TPK_ID_Bank")%>','<%= rs("TPK_Nip") %>','<%= rs("TPK_Tanggal") %>','<%= rs("Kry_Nama") %>','<%= rs("TPK_Ket") %>','<%= rs("TPK_pp") %>','<%= rs("TPK_Bunga") %>','<%= rs("TPK_lama") %>','<%= rs("TPK_PotongGajiYN") %>')" data-bs-toggle="modal" data-bs-target="#modalPimjaman">Edit</button>
                        <%end if%>
                        <%if session("HT3AC") = true then%>
                          <% if rs("TPK_AktifYN") = "Y" then %>
                            <button type="button" class="btn btn-danger btn-sm py-0 px-2" onclick="return aktifPinjaman('<%=rs("TPK_ID_Bank")%>','<%= rs("TPK_AktifYN") %>','<%= rs("TPK_Nip") %>')">NonAktif</button>
                          <% else %>
                            <button type="button" class="btn btn-warning btn-sm py-0 px-2" onclick="return aktifPinjaman('<%=rs("TPK_ID_Bank")%>','<%= rs("TPK_AktifYN") %>','<%= rs("TPK_Nip") %>')">Aktif</button>
                          <% end if %>
                        <% end if %>
                        <%if session("HT3AD") = true then%>
                          <button type="button" class="btn btn-secondary btn-sm py-0 px-2" onclick="window.open('EXPORT-Pinjaman.asp?p=<%= rs("TPK_ID_Bank") %>')">Cetak</button>
                        <%end if%>
                      </div>
                    </td>
                </tr>

...
<% 
	...
	if p = "" OR p = "Y" then
		karyawan_cmd.commandText ="SELECT * from HRD_M_Karyawan WHERE (ISNULL(Kry_DDBID, '') <> '') and Kry_AktifYN = 'Y' AND Kry_Nip NOT LIKE '%A%' AND Kry_Nip NOT LIKE '%H%'"
		set karyawan = karyawan_cmd.execute
	else
		karyawan_cmd.commandText ="SELECT * from HRD_M_Karyawan WHERE (ISNULL(Kry_DDBID, '') <> '') and Kry_AktifYN = 'N' AND Kry_Nip NOT LIKE '%A%' AND Kry_Nip NOT LIKE '%H%'"
		set karyawan = karyawan_cmd.execute
	end if
	...
	if p = "" OR p = "Y" then
		sqlawal = "SELECT * from HRD_M_Karyawan WHERE (ISNULL(Kry_DDBID, '') <>'') and Kry_AktifYN = 'Y' AND Kry_Nip NOT LIKE '%A%' AND Kry_Nip NOT LIKE '%H%'"
	else
		sqlawal = "SELECT * from HRD_M_Karyawan WHERE (ISNULL(Kry_DDBID, '') <>'') and Kry_AktifYN = 'N' AND Kry_Nip NOT LIKE '%A%' AND Kry_Nip NOT LIKE '%H%'"
	end if
...
<!--#include file="landing.asp"-->
	<div class="container">
		<div class='row mb-3'>
			<div class='col'>
				<h2 class="mt-3 mb-1 text-center">DAFTAR KARYAWAN</h2>
			</div>
		</div>
		<div class="row">
			<div class="col md-3">
					<div class='row'>
						...
						<% if session("HA1E") = true then %>
							<a class ="btn btn-danger mb-2 updateNip" href="<%=url%>/updateNip.asp">Update Nip</a>
						<% end if %>	
							<a class ="btn btn-secondary mb-2 kryHarian" href="<%=url%>/karyawanharian.asp">Karyawan Harian</a>
						</div>
					</div>
			<form action="ajax/CariKaryawan.asp" method="post" name="carikaryawan" id="carikaryawan">
					...
					<table class="table table-dark table-striped" cellpadding="10" cellspacing="0" id="table" style="font-size:14px;">
							...
							<tr>
								...
								<th>
									<% if orderBy = "ORDER BY Kry_ActiveAgenID ASC" then %>
										<a href="index.asp?c=OBK_N&p=<%= p %>" style="text-decoration:none;color:#fff;font-size:14px;"><i class="fa fa-arrow-circle-o-down" aria-hidden="true"></i> PENGGAJIAN DI</a>
									<% else %>
										<a href="index.asp?s=OBK_A&p=<%= p %>" style="text-decoration:none;color:#fff;font-size:14px;"><i class="fa fa-arrow-circle-o-up" aria-hidden="true"></i> PENGGAJIAN DI</a>
									<% end if %>
								</th>
								<th>
						...
						%>
							<tr>
								...
								%> 
								<td><%= cbg%></td>
								<td><a href="forms/activeAgen.asp?nip=<%= rs("Kry_Nip") %>" style="color:#fff;text-decoration:none;"><%= aktifcbg %></a></td>
								...
								<td>
									<a href="detail-karyawan/index.asp?nip=<%= rs("Kry_NIP")%>" class="btn btn-outline-info btn-sm btn-detail" name="detail">Detail</a>
								</td>
							</tr>
						<%

...
<% 
dim urut, tgla, tgle, mutasi, mutasi_cmd
dim orderby
...
<div class="btn-group position-absolute top-0 end-0" role="group" aria-label="Basic outlined example">
  <button type="button" class="btn btn-outline-primary btn-sm" onClick="window.open('exportXls-karyawanmutasi.asp?urut=<%=urut%>&tgla=<%=tgla%>&tgle=<%=tgle%>','_self')">EXPORT</button>
</div>
<div class='row'>
            ...
            if kelamin = "W" then
             ...
             %>
                <tr>
                    ...
                    <td><%=nomor%></td>
                    <td><%=mutasi("Mut_Nip")%></td>
                    <td><%=mutasi("Kry_Nama")%></td>
                    <td><%=agenl%></td>
                    <td><%=ljabatan%></td>
                    <td><%=agenbaru("agen_nama")%></td>
                    <td><%=njabatan%></td>
                    <td><%=replace(formatCurrency(cdbl(gapokl)),"$","Rp.")%></td>
                    <td><%=replace(formatCurrency(cdbl(tunjl)),"$","Rp.")%></td>
                    <td><%=replace(formatCurrency(cdbl(gapokn)),"$","Rp.")%></td>
                    <td><%=replace(formatCurrency(cdbl(tunjn)),"$","Rp.")%></td>
                    <td><%=replace(formatCurrency(cdbl(selisihgaji)),"$","Rp.")%></td>
                    <td><%=replace(formatCurrency(cdbl(selisihtunjangan)),"$","Rp.")%></td>
                    <td></td>

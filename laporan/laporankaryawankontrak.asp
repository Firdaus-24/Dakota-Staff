...
<% 
dim laporan, urut, area, pegawai, bank, status, bulan, tahun, karyawanout, simvalid
dim agen_cmd, agen
...
<div class="btn-group position-absolute top-0 end-0" role="group" aria-label="Basic outlined example">
  <button type="button" class="btn btn-outline-primary btn-sm" onClick="window.open('exportXls-daftarkaryawankontrak.asp?urut=<%=urut%>&tgla=<%=tgla%>&tgle=<%=tgle%>&area=<%=area%>&pegawai=<%=pegawai%>','_self')">EXPORT</button>
<% end if %>
</div>
    <div class='row'>

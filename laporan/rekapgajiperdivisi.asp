...
bulan = month(Request.Form("tgla"))
tahun = year(Request.Form("tgla"))
...
<div class='row'>
    <div class='col text-center judul'>
        <label class="text-center">REKAP GAJI PERDIVISI</label></br>
        <label class="text-center">PRIODE <b> <%= Ucase(monthName(bulan)) &" "& tahun %></b></label>
    </div>
</div>
<div class='row'>
...
<table class="table table-striped" style="font-size: 10px; display: block;width: 100%;overflow: scroll;">
    <tr>
        <th>Divisi</th>
        <th>Gapok</th>
        <th>Insentif</th>
    ...
    gapok = FormatCurrency(karyawan("totalgapok"))
    insentif = FormatCurrency(karyawan("totalinsentif"))
    transport = FormatCurrency(karyawan("totaltransport"))
    kesehatan = FormatCurrency(karyawan("totalkesehatan"))
    keluarga = FormatCurrency(karyawan("totalkeluarga"))
    jabatan = FormatCurrency(karyawan("totaljabatan"))
    jamsostek = FormatCurrency(karyawan("totaljamsostek"))
    pph21 = FormatCurrency(karyawan("totalpph21"))
    pinjaman = FormatCurrency(karyawan("totalpinjaman"))
    koperasi = FormatCurrency(karyawan("totalkoperasi"))
    klaim = FormatCurrency(karyawan("totalklaim"))
    asuransi = FormatCurrency(karyawan("totalasuransi"))
    persekot = FormatCurrency(karyawan("totalpersekot"))
    absen = FormatCurrency(karyawan("totalabsen"))
    lain = FormatCurrency(karyawan("totallain"))
    thr = FormatCurrency(karyawan("totalthr"))

    tgapok = (Replace(gapok,"$","Rp."))
    tinsentif = (Replace(insentif,"$","Rp."))
    ttransport = (Replace(transport,"$","Rp."))
    tkesehatan = (Replace(kesehatan,"$","Rp."))
    tkeluarga = (Replace(keluarga,"$","Rp."))
    tjabatan = (Replace(jabatan,"$","Rp."))
    tjamsostek = (Replace(jamsostek,"$","Rp."))
    tpph21 = (Replace(pph21,"$","Rp."))
    tpinjaman = (Replace(pinjaman,"$","Rp."))
    tkoperasi = (Replace(koperasi,"$","Rp."))
    tklaim = (Replace(klaim,"$","Rp."))
    tasuransi = (Replace(asuransi,"$","Rp."))
    tpersekot = (Replace(persekot,"$","Rp."))
    tabsen = (Replace(absen,"$","Rp."))
    tlain = (Replace(lain,"$","Rp."))
    tthr = (Replace(thr,"$","Rp."))
    %>
    <tr>
        ...
        <td><%=tgapok%></td>
        <td><%=tinsentif%></td>
        <td><%=replace(formatcurrency(karyawan("pengembalianpot")),"$","Rp.")%></td>
        <td><%=replace(formatcurrency(karyawan("pphdtp")),"$","Rp.")%></td>
        <td><%=ttransport%></td>
        <td><%=tkesehatan%></td>
        <td><%=tkeluarga%></td>
        <td><%=tjabatan%></td>

...
Response.AddHeader "content-disposition", "filename=Laporan Gaji Percabang " &  Request.QueryString("tgla") & ".xls"
%>
...
<body>
    ...
    <tr>
        <td colspan="23" style="text-align:center;">
            PERIODE <b><%= Ucase(MonthName(bulan)) & " " & tahun %></b>
        </td>
    </tr>
    <tr>
        ...
        set karyawan = karyawan_cmd.execute
            ...
            tpendapatan = replace(ptpendapatan,"$","")
             ...
             %>
                <tr>
                    <% if nonBPJS >= 1 then %>
                    <td  class="table-active" colspan="3"><%=agen("agen_nama")%> <span style="color:red;font-size:12px;"> <%=nonBPJS%> karyawan belum diupdate BPJS</span></td>
                    <%else %>
                    <td  class="table-active" colspan="3"><%=agen("agen_nama")%></td>
                    <%end if %>
                    <% nonBPJS = 0 %>

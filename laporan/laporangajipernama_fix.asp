...
<head>
    ...
    <title>LAPORAN GAJI PERNAMA</title>
...
<div class="d-grid gap-2 d-md-flex justify-content-md-end">
    ...
    <div class="btn-group" role="group" aria-label="Basic outlined example">
    <button type="button" class="btn btn-outline-primary btn-sm" onClick="window.open('exportXls-gajipernama.asp?bulan=<%=bulan%>&tahun=<%=tahun%>&area=<%=area%>','_self')">EXPORT</button>
    </div>
</div>
<div class='row'>
    ...
    <div class='row'>
        ...
        <table class="table" style="font-size: 10px; display: block;width: 100%;overflow: scroll;">
            ...
            subtotalpphdtp = subtotalpphdtp + Clng(karyawan("Sal_InsentifPPh21DTP"))
             %>
                <tr>
                    ...
                    <td><%= replace(formatCurrency(bpjsp),"$","") %></td>
                    <td><%= transport %></td>
                    <td><%= kesehatan %></td>
                    <td><%= keluarga %></td>
                    <td><%= jabatan %></td>
                    ...
                    <td><%= koperasi %></td>
                    <td><%= klaim %></td>
                    <td><%= replace(formatCurrency(bpjsk),"$","") %></td>
                    <td><%= absen %></td>
                    <td><%= lain %></td>
                    <td><%= asuransi %></td>
                    <td><%= potongan %></td>
            ...
            tpendapatan = replace(ptpendapatan,"$","")
             ...
             %>
                <tr>
                    <td  class="table-active" colspan="3">SUBTOTAL <%=agen("agen_nama")%></td>

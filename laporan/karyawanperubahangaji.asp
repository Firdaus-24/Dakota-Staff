...
<div class="btn-group position-absolute top-0 end-0" role="group" aria-label="Basic outlined example">
  <button type="button" class="btn btn-outline-primary btn-sm" onClick="window.open('exportXls-perubahangaji.asp?urut=<%=urut%>&tgla=<%=tgla%>','_self')">EXPORT</button>
</div>
    <div class='row'>
        ...
        <table class="table table-bordered text-center" style="font-size: 12px;" >
            ...
            %>
                <tr>
                    ...
                    <td><%= replace(formatcurrency(cdbl(gapok)),"$","Rp.") %></td>
                    <td><%= replace(formatcurrency(cdbl(tunjangan)),"$","Rp.") %></td>
                    <td><%= replace(formatcurrency(cdbl(karyawan("sal_gapok"))),"$","Rp.") %></td>
                    <td><%= replace(formatcurrency(cdbl(karyawan("sal_tunjJbt"))),"$","Rp.") %></td>
                    <td><%= replace(formatcurrency(cdbl(selisihgapok)),"$","Rp.") %></td>

...
<% 
    ...
    laparea = trim(Request.Form("laparea"))
    lappegawai = trim(Request.Form("lappegawai"))
...
<body>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
        <div class="btn-group" role="group" aria-label="Basic example">
            ...
            <button type="button" class="btn btn-outline-primary btn-sm" onClick="window.open('exportXls-ldkkaryawan.asp?tgla=<%=tgla%>&tgle=<%=tgle%>&area=<%=area%>','_self')">EXPORT</button>
        </div>
    </div>
    <div class='row'>
    ...
    <div class='row'>
        <div class='col text-center judul'>
            <label class="text-center">IZIN, CUTI, SAKIT DAN ALFA</label></br>
            <label class="text-center">PERIODE :<b><%= tgla & " S/D " & tgle %></b></label>
        </div>
    </div>
    <div class='row'>
            ...
            <table class="table" style="font-size:12px;">
                ...
                <tbody>
                    <% do while not detailcuti.eof %>
                        <tr>
                            <td>CABANG</td>
                            <td colspan="14"><%= detailcuti("Agen_Nama") %></td>
                        </tr>
                        <% 

...
<% 
	...
	cust_cmd.commandtext = "SELECT Cust_ID, Cust_Name FROM MKT_M_Customer WHERE (Cust_ID = '5610000004')"
	set cust = cust_cmd.execute

	Dim uploadsDirVar
	uploadsDirVar = "D:\newsite\dbs\csvdata\" & cust.fields.item("cust_ID") & "\"
	'uploadsDirVar = "D:\newsite\trial\csvdata\"
	...
	function SaveFiles
		...
		if (UBound(ks) <> -1) then
			...
			for each fileKey in Upload.UploadedFiles.keys
				...
				'response.Redirect("csvdata/readcsv.asp?sFileName="&Upload.UploadedFiles(fileKey).FileName&"&cID="+b)
				'response.Redirect("csvdata/readcsv_rackindo.asp?sFileName="&Upload.UploadedFiles(fileKey).FileName)
				response.Redirect("csvdata/readcsv_skv.asp?sFileName="&Upload.UploadedFiles(fileKey).FileName)
			next
		else
...
<div class="wrap-70">
	<div class="row">
		<header class="kepala">
			<div class="kep-jud"><h1>UPLOAD CSV SKV SEJAHTERA</h1></div>
		</header>
	</div>
	<hr />


	<form name="frmSend" method="POST" enctype="multipart/form-data" action="mkt_t_econote_csv_upload_skv.asp" onSubmit="return onSubmitForm(this);">   	
	...
	<legend>UPLOAD CSV</legend>
		...
		<!--
			...
			<input size="35" id="CustID" name="CustID" type="text" onKeyDown="showCustomer(this.value);" onKeyPress="showCustomer(this.value)" onKeyUp="showCustomer(this.value);"  onClick=" pengirim();" tabindex="1" onBlur="hideTxtHint();"  />
			<input type="text" id="custIDNomor" name="custIDNomor" size="8"  readonly> 
			<br>
			<label></label>
			<div id="txtcustomer">
			</div><br>
		-->
			...
			<label>Pilih File CSV untuk customerID <%=cust("Cust_ID")%>: </label>
			<div class="row space">
	...
	<input id="tombollihat" type="button" value="Daftar Barang Belum Dibuat BTT" onClick="window.open('mkt_t_econote_csv_upload_skv_d.asp?vcustname=<%=cust("Cust_Name")%>&vcustid=<%=cust("Cust_ID")%>','_self')" class="tombol tombolgray full_12">
	</div>
	</form>
